# Rekomendasi `.env` Production — PRODULI Backend

Domain yang jadi acuan dokumen ini:

- **Backend (Laravel, repo ini):** `api.produli.labkesdasumenep.id`
- **Frontend (Nuxt):** `produli.labkesdasumenep.id`
...
APP_URL=https://api.produli.labkesdasumenep.id
...
PRODULI_INTEGRATION_SECRET=__ISI_MANUAL__
PRODULI_NIK_HASH_SECRET=__ISI_MANUAL__
...
GOOGLE_REDIRECT_URI=https://api.produli.labkesdasumenep.id/auth/google/callback
...
PRODULI_FACE_DETECTION_ENABLED=false
PRODULI_GPS_MAX_ACCURACY_METERS=100
PRODULI_GPS_MAX_AGE_SECONDS=300
PRODULI_GEOFENCE_RADIUS_VERIFIED=150
PRODULI_GEOFENCE_RADIUS_APPROXIMATE=3000
PRODULI_EXIF_MAX_AGE_SECONDS=300
PRODULI_EXIF_GPS_TOLERANCE_METERS=200

# --- Reminder kader H-1/hari-H (docs/planning/02 §8)
PRODULI_REMINDER_H1_TIME=16:00
PRODULI_REMINDER_SAME_DAY_TIME=06:00
...
| `APP_URL` | `http://localhost:8033` | `https://api.produli.labkesdasumenep.id` | Domain asli |
| `FRONTEND_URL` | `http://localhost:3033` | `https://produli.labkesdasumenep.id` | Domain asli, dipakai juga untuk `allowed_origins` di `config/cors.php` |
| `GOOGLE_REDIRECT_URI` | `http://localhost:8033/auth/google/callback` | `https://api.produli.labkesdasumenep.id/auth/google/callback` | Harus terdaftar juga di Google Cloud Console |
...
- `PRODULI_FACE_DETECTION_ENABLED=false` — keputusan arsitektur v1 (docs/planning/02 §10), bukan
  soal environment.
