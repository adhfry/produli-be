# Dokumen Perencanaan 2/3 — Arsitektur Backend PRODULI (Laravel 11)
...
| Tabel                   | Fungsi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`                 | Semua pengguna PRODULI; role via Spatie Permission                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `puskesmas`             | **Dikelola manual di PRODULI**, entitas operasional — SiLAKES tidak punya level puskesmas sama sekali. Tidak punya FK ke kecamatan (lihat §2a). **Kolom tambahan (§15):** `alamat` (text), `no_telp`, `no_wa` (nullable), `latitude`/`longitude` (nullable), `deskripsi` (text nullable) — TETAP tabel `puskesmas`, JANGAN digeneralisasi jadi tabel `instansi` polimorfik (`puskesmas_id` sudah dipakai luas di seluruh sistem, refactor itu risiko tinggi untuk manfaat yang belum nyata — kalau perlu jenis instansi lain nanti, itu keputusan terpisah)                                                                                                                                                                                  |
...
| `wilayah_mapping`       | Rekonsiliasi teks bebas `kel_desa`/`kecamatan` dari data pasien SiLAKES → `desa_id` baku PRODULI. **Wajib** karena field ini di SiLAKES masih hasil input manual, belum berupa kode wilayah                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `out_of_scope` | `kecamatan_raw` tidak match ke salah satu kecamatan Kabupaten Sumenep (pasien dari luar daerah) | Dikecualikan dari program PRODULI — bukan backlog kerja, jangan masuk antrean review                                    |
...
- **Keputusan terbaru: tidak ada auto-apply sama sekali.** Semua usulan dari PRODULI — geo, kontak/alamat, maupun identitas — masuk `pending_review` dan baru berlaku setelah disetujui staf Labkesda lewat **halaman persetujuan** baru di SiLAKES. Ini kebijakan Dinkes: data pasien pemerintah wajib diverifikasi manusia, tidak dibedakan lagi per kategori risiko field seperti rancangan sebelumnya (geo pun sekarang ikut direview, bukan otomatis diterima).
- **Urutan kerja:** ini butuh kerja paralel di **kedua repo** — SiLAKES membangun endpoint penerima + halaman persetujuan (Dokumen 1 §9), PRODULI membangun sisi pengirim di `VisitReportService` (mini-form konfirmasi lokasi + lengkapi data, sudah dirancang di atas) yang memanggil endpoint ini setelah laporan kunjungan tersimpan lokal. Keduanya bisa dikerjakan bersamaan, tidak saling blocking — PRODULI tetap simpan `geo_status`/`latitude`/`longitude` secara lokal dulu meski panggilan ke SiLAKES gagal/tertunda (queue job dengan retry, bukan synchronous call yang bisa menggagalkan submit laporan kunjungan kader). PRODULI tidak perlu polling status approval — begitu disetujui, sinkronisasi rutin (`updated_at` delta sync) otomatis menangkap perubahan di siklus berikutnya.
...
| `RiskClassificationService` | Hitung level risiko dari `lab_results_cache` × `risk_thresholds`, simpan snapshot. **Wajib pakai hasil lab TERBARU per pasien berdasarkan `tanggal` (tanggal pemeriksaan medis), bukan urutan kedatangan sync** — delta sync tidak menjamin urutan kronologis (hasil lama bisa saja tersinkron belakangan). Query: `MAX(tanggal)` per `patient_id` di `lab_results_cache`, baru klasifikasi dari baris itu — jangan asumsikan baris yang baru disync = baris terbaru secara medis. **Kriteria (REVISI KEDUA — laporan bug A. Jazili, Trigliserida 192 sendirian salah naik ke Sedang; AND boolean penuh per tier, BUKAN "salah satu parameter saja cukup"):**<br>`Berat` = `GDP>120 AND CholesterolTotal>200 AND Trigliserida>140 AND LDL>130 AND Urea>46` — kelimanya harus ADA HASILNYA dan semuanya melebihi sekaligus. Kalau satu saja datanya belum ada atau normal, kondisi ini otomatis tidak terpenuhi (bukan Berat).<br>`Sedang` = pola IDENTIK dengan Berat, `GDP>120 AND CholesterolTotal>200 AND Trigliserida>140 AND LDL>130` (4 parameter, tanpa Urea) — keempatnya harus ADA HASILNYA dan semuanya melebihi sekaligus. TIDAK ADA LAGI pelonggaran "minimal 1 dari 4" atau fallback ke `Ringan` untuk kombinasi parsial (mis. cuma GDP+Cholesterol, atau cuma Trigliserida sendirian) — kombinasi yang tidak AND-lengkap dianggap belum cukup jadi perhatian sama sekali, jatuh ke `tidak_berisiko`/null seperti biasa (bukan `Ringan`), keputusan eksplisit user saat laporan bug ini.<br>`Creatinine` BUKAN bagian kombinasi 4/5-parameter di atas — "direct classifier" bertingkat (`risk_thresholds.is_direct_classifier=true`, 1.7–1.9=Sedang, ≥2.0=Berat) yang bisa menentukan level SENDIRIAN tanpa parameter lain ikut melebihi (revisi Bu Kadis). Level akhir = level TERPARAH antara jalur kombinasi 4/5-parameter dan jalur direct-classifier Creatinine manapun yang match.<br>`tidak_berisiko` ditulis (baris baru, bukan overwrite) saat pasien yang PERNAH diklasifikasi membaik/tidak lagi memenuhi kriteria tier manapun (revisi Bu Kadis) — pasien yang BELUM PERNAH diklasifikasi dan tidak match apa pun tetap tidak ditulis sama sekali (bukan noise).<br>`Ringan` sejak revisi ini TIDAK PERNAH lagi dihasilkan algoritma ini (dulu tier khusus "hanya GDP sendirian") — tetap valid sebagai nilai enum kolom `level` untuk baris historis lama, cuma tidak diproduksi lagi.<br>Pasien `Berat` DAN `Sedang` jadi kandidat kunjungan kader (lihat §12 — diperluas dari cuma `Berat` atas permintaan Bu Kadis) |
| `VisitAssignmentService`    | Assign kader ke pasien, validasi ketersediaan kader, buat reminder                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `VisitValidationService`    | 7-layer validation — **rekomendasi Strategy Pattern per layer** (`GpsActiveCheck`, `GeofenceCheck`, `LiveCameraCheck`, `WatermarkGenerator`, `ExifValidator`, `FaceDetectionCheck`, `OfflineQueueHandler`) agar tiap layer independen, testable, dan bisa di-toggle sesuai kebijakan (Open/Closed Principle). **`FaceDetectionCheck` non-aktif secara default (feature-flag `config('produli.validation.face_detection_enabled')`)** — lihat §10                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
_TODO(infra): bucket/domain MinIO di bawah ini masih pakai penamaan `kopipu` lama — belum
di-rename ke penamaan berbasis `produli` karena ini infra live yang sedang dipakai upload foto
sungguhan. Update bagian ini begitu rename infra selesai dikerjakan terpisah._
...
- `.env`: `AWS_ACCESS_KEY_ID=kopipu-laravel`, `AWS_BUCKET=kopipu`, `AWS_ENDPOINT=https://storage.kopipu-smart.labkesdasumenep.cloud`, `AWS_USE_PATH_STYLE_ENDPOINT=true` (wajib true untuk MinIO, beda dari AWS S3 asli). **`FILESYSTEM_DISK` tetap `local`** (default framework, jangan diubah global) — pakai env var bernama khusus (`PRODULI_VISIT_PHOTOS_DISK=s3`) untuk fitur yang benar-benar butuh S3, bukan mengubah default seluruh aplikasi; tidak semua yang Laravel simpan (log, cache file) perlu masuk S3.
...
**Keputusan final SameSite cookie (domain prod sudah dikonfirmasi):** backend `api.produli.labkesdasumenep.id` adalah subdomain dari frontend `produli.labkesdasumenep.id` — keduanya berbagi **registrable domain/eTLD+1 yang sama** (`labkesdasumenep.id`), jadi secara teknis ini **same-site**, bukan cross-site. **`SameSite=Lax` sudah cukup dan benar untuk prod** — tidak perlu `SameSite=None` (yang mewajibkan HTTPS + punya risiko lebih longgar). Syarat teknis supaya ini benar-benar jalan:

- Cookie refresh token biarkan **host-only** (jangan set `Domain` attribute lebar ke parent) — karena cookie ini cuma perlu balik ke `api.produli.labkesdasumenep.id`, bukan ke frontend.
- CORS di Laravel: `Access-Control-Allow-Origin` harus origin spesifik (`https://produli.labkesdasumenep.id`), **bukan wildcard `*`** — tidak bisa pakai wildcard bersamaan dengan `Access-Control-Allow-Credentials: true`.
...
- `GOOGLE_REDIRECT_URI` ganti ke `https://api.produli.labkesdasumenep.id/api/v1/auth/google/callback`, `FRONTEND_URL` ke `https://produli.labkesdasumenep.id`.
...
- **Layer 6 (Face Detection) — keputusan: non-aktif secara default di v1.** 6 layer lain (GPS, geofencing, live camera, watermark, EXIF, offline queue) sudah memberi anti-fraud yang kuat; face detection menambah kompleksitas (model deteksi, beban PWA di HP low-end kader) untuk kenaikan keamanan yang marginal.
    - Implementasi tetap dibuat sebagai strategy terpisah (`FaceDetectionCheck`) di belakang feature-flag `config('produli.validation.face_detection_enabled')` supaya bisa diaktifkan kapan saja tanpa refactor.
...
**Alur:** PJ buka `/dashboard/kunjungan` → filter pasien kandidat (level risiko **Berat dan Sedang** — diperluas dari cuma Berat atas permintaan Bu Kadis, scope puskesmasnya, opsional filter per desa + cari nama/no. registrasi) → daftar tampil dengan checkbox berlabel level risiko per pasien, ada opsi "Pilih Semua yang Belum Ditugaskan" → PJ pilih kader tujuan + centang pasien (bisa banyak sekaligus) → modal konfirmasi menampilkan daftar nama yang akan jadi target → submit → sistem buat banyak `visit_assignments` sekaligus (1 per pasien terpilih, kader sama).
...
_Lanjut ke Dokumen 3: UI/UX & Frontend Nuxt PRODULI._
